...
**Methods.** We describe a deployed system — Sentinel Health — that uses Gemma 4 (`e4b-it-q4_K_M`) as the inference workhorse for offline emergency triage in rural Tamil Nadu, India, and we report preliminary head-to-head observations against Gemini 2.7 Flash on a representative failure case (image-only suspected snake bite). We then specify a pre-registered research plan to evaluate eight small open-weight models and three frontier proprietary models on a curated 250-case clinical triage benchmark spanning five emergency categories (Trauma, Poisoning, Snake Bite, Myocardial Infarction, Stroke) and four languages (English, Hindi, Tamil, Malayalam). Primary endpoint: triage-class sensitivity for RED-tier cases. Secondary endpoints: specificity, calibration (expected calibration error), per-case latency, per-case cost, multimodal accuracy on image-bearing cases, and adversarial robustness to prompt injection.

**Preliminary Results.** On a 31-case pilot eval using Gemma 4 e4b alone, sensitivity for RED-tier text-described cases was 100% (29/29); specificity was 80% (29/31). The same sensitivity figure held across two MedGemma 4B variants tested, providing initial evidence that within the Sentinel architecture the model is *substitutable* and the deterministic safety net is the load-bearing component. On a single image-only suspected snake-bite case, Gemma 4 e4b returned YELLOW with "no acute condition identified"; Gemini 2.7 Flash, presented with the same image and minimal textual context, returned a structured emergency response with correct first-aid guidance.

**Conclusions.** The framing of "SLM versus frontier" is incomplete without specifying the pipeline. For text-described triage in a curated narrow scope, SLM-plus-deterministic-safety-net is competitive with frontier models, replaceable across model choices, and dramatically cheaper to deploy at population scale. For image-only or compound contextual reasoning, frontier models retain a substantive lead that cannot be closed by prompt engineering alone. The proposed research plan will quantify these gaps across eleven models and prescribe specific architectural patterns (two-pass vision-as-sensor, KB-grounded JSON-Schema output, selective frontier escalation) that allow SLMs to carry production traffic where their intrinsic capability would not.
...
This paper has three contributions. First, we describe a deployed clinical-triage system, *Sentinel Health*, that uses an open-weight 8 B-parameter model (Gemma 4 `e4b-it-q4_K_M`) as the inference workhorse and we report concrete preliminary results from its evaluation set, including a representative failure case where Gemma 4 fails and a frontier model (Gemini 2.7 Flash) succeeds. Second, we pre-register a detailed research plan to evaluate eleven candidate models — eight small open-weight, three frontier proprietary — across the clinical scope of Sentinel, with hypotheses, primary and secondary outcomes, statistical methodology, and a power analysis. Third, we propose an architectural taxonomy of *compensations* — deterministic safety nets, KB-grounded JSON-Schema output, two-pass vision-as-sensor pipelines, and selective frontier escalation — that we hypothesize allow small open-weight models to carry production clinical traffic where the unaided model would not.

The central thesis is that "small versus frontier" framed as a model comparison is a malformed question. The right question is *which pipeline composition*, parameterized by both the model and the deterministic scaffolding around it, satisfies the clinical task within the deployment constraints. The Sentinel architecture provides one concrete proposal for what such a pipeline looks like; the research plan in §9 provides a methodology for testing it rigorously across model classes.
...
Three classes, eleven models total. Selection criteria: (a) open-weight or accessible-API by April 2026; (b) declared support for English plus at least one Indic language *or* established multilingual capability; (c) for SLMs, fits within 24 GB RAM at deployment quantization.

| Class | Model | Params | Quantization | Distribution |
|---|---|---|---|---|
| Frontier | Gemini 2.7 Pro | undisclosed | n/a (API) | Google Cloud |
| Frontier | Gemini 2.7 Flash | undisclosed | n/a (API) | Google Cloud |
...
The 11 × 3 design yields 33 model-configuration cells. The full benchmark (§6.3) is run in each cell.
...
For each of the 33 model-configuration cells:
...
**Case A: Suspected snake bite (Figure 1).** Gemma 4 e4b returned YELLOW with "No acute condition identified" when the text was "patient brought in with this wound on forearm, child screaming." The same image, presented to Gemini 2.7 Flash with text "I was sleeping outside and my hand began hurt very strongly", returned a structured emergency response with correct first-aid guidance. The full Gemini response is reproduced in §7.5. **The Gemma-4 visual module recognized the lesions but did not connect them to snake bite envenomation without an explicit textual cue.**
...
### 7.4 Substitutability across SLMs (informal)

### 7.4 Substitutability across SLMs (informal)

In the development phase, the same 31 cases were run against MedGemma 4 B Q8 and MedGemma 4 B Q6 in addition to Gemma 4 e4b. The RED-tier sensitivity figure was 29/29 in all three. Specificity varied (Gemma 4: 29/31; MedGemma Q8: 28/31; MedGemma Q6: 27/31), suggesting that the safety net dominates the sensitivity outcome while the model's intrinsic capability is reflected in the specificity outcome.
...
To corroborate H2 with prospective rather than purely anecdotal data, we ran a 9-case pilot against three frontier providers (Anthropic Claude Opus 4.7, OpenAI GPT-5, Google Gemini 2.5 Flash) with identical inputs to the Gemma 4 testing in §7.3. The 9 cases comprise Cases A1/A2 (snake-bite image with two phrasings), Cases B1/B2 (redacted ECG image with two phrasings), and five text-only RED audit-log cases spanning all five emergency categories. All 27 calls were issued at temperature 0 with the same system prompt (Appendix E). The raw results are persisted at `data/frontier_pilot/results.jsonl`; the reproduction script is at `scripts/frontier_pilot.py`.
...
The gap with Gemma 4 e4b on the same image and comparable text is consistent with the prevailing literature on multimodal compositional reasoning at scale [@gemini27report; @anthropicvalidation2025] and with the present pilot's empirical findings.
...
The four patterns compose. DSN is necessary for the substitutability claim. KGS is necessary for the safety of KB-bounded output. VAS extends DSN to image inputs. SFE adds a frontier safety-net at the cost of network dependency. Sentinel deploys DSN + KGS in v0.1; we propose DSN + KGS + VAS for v0.2 (subject to the H4 evaluation result); SFE remains an out-of-scope option for product variants that target different deployment surfaces.
...
If the substitutability claim (H1) holds in the formal evaluation, the implication is that population-scale clinical AI deployments in low-resource settings can be priced as one-time hardware procurements rather than recurring API subscriptions. At Indian NHM scale (~1 million ASHAs), the difference between a $0.0001/query frontier API call (favorable estimate) and a near-zero per-call SLM is, by our calculation, ~$25–50 M per year in API spend forestalled. The deciding constraint becomes hardware reliability and operator training, not model cost.
...
3. **The architectural taxonomy is proposed, not empirically validated.** Patterns 1 and 2 are deployed in Sentinel; Patterns 3 and 4 are not, and their hypothesized effects (H4 in particular) await the planned evaluation.

4. **The cost analysis depends on assumed query volumes.** Population-scale deployment numbers (~5 queries/CHW/week × ~1M CHWs) are estimates; actual operational query volume could be 10× higher or 5× lower depending on adoption.

5. **The frontier-model API access landscape changes faster than the publication cycle.** Model versions pinned in §6.2 may be deprecated by the time the evaluation completes. We commit to running against the latest available version of each frontier provider at the time of the held-out replication run, and to re-running the analysis if version drift exceeds a pre-specified threshold.

6. **No formal cost-effectiveness or QALY analysis is conducted.** The clinical outcome of interest in Sentinel is reduced time-to-physician-decision, but the link to mortality reduction or quality-adjusted life years is not measured here. Such a study would be a follow-up downstream of the present plan.

7. **The image-only failure observation in §7.3 is a single case.** It is presented to motivate H2 and H3, not to substantiate them. Even an n = 1 observation, if dramatic enough, is worth reporting as motivating evidence; we do not treat it as conclusive.
...
**Fine-tuning analysis.** A formal evaluation of whether domain fine-tuning (e.g., MedGemma-style) on Sentinel's KB and red-flag rule set improves the SLM's intrinsic capability sufficiently to weaken the DSN's contribution. If so, the scaffolding can be lighter; if not, the scaffolding's central role is reinforced.
...
We have proposed a multi-model, pre-registered evaluation across eleven candidate language models, six clinical-scope categories, four target languages, and 250 cases, with seven explicit hypotheses, a defined statistical methodology, a sample-size justification, an ethics-and-data-handling specification, and a 30-week timeline. We have also proposed an architectural taxonomy — deterministic safety net, KB-grounded JSON Schema, two-pass vision-as-sensor, selective frontier escalation — as the framework within which small and frontier models compose.
...
[@gemini27report] Google DeepMind. *Gemini 2.7: A Family of Highly Capable Multimodal Models.* Technical Report, 2026.
